...
function get_categories_view() {
	...
	$categories['main_list'] = sql_fetch_all('SELECT * FROM '.$mysqli_table_prefix.'categories WHERE parent_num=0 ORDER BY category');
		
	if (is_array($categories['main_list'])) {
		foreach ($categories['main_list'] as $_key => $_val) {
			$categories['main_list'][$_key]['sub'] =  sql_fetch_all('SELECT * FROM '.$mysqli_table_prefix.'categories WHERE parent_num='.$_val['category_id']);
		}
	}
	return $categories;
}
...
function get_category_info($catid) {
	global $mysqli_table_prefix;
	$categories['main_list'] = sql_fetch_all("SELECT * FROM ".$mysqli_table_prefix."categories ORDER BY category");
	...
	if (is_array($categories['subcats'])) {
		foreach ($categories['subcats'] as $_key => $_val) {
			$categories['subcats'][$_key]['count'] = sql_fetch_all('SELECT count(*) FROM '.$mysqli_table_prefix.'site_category WHERE 	category_id='.(int)$_val['category_id']);
		}
	}
