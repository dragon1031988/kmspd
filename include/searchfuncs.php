...
//		$a = remove_accents($a);
	...
	function search($searchstr, $category, $start, $per_page, $type, $domain) {
		global $length_of_link_desc,$mysqli_table_prefix, $show_meta_description, $merge_site_results, $stem_words, $did_you_mean_enabled ;

		$possible_to_find = 1;
		$result = mysqli_query($GLOBALS['connect'], "select domain_id from ".$mysqli_table_prefix."domains where domain = '$domain'");
		...
		while ($phrase_words < count($wordarray)) {
			...
			$query1 = "SELECT link_id from ".$mysqli_table_prefix."links where fulltxt like '% $searchword%'";
		...
		if ((count($result_array_full) == 0 || $possible_to_find == 0) && $did_you_mean_enabled == 1) {
			...
			foreach ($searchstr['+'] as $word) {
				...
				$result = mysqli_query($GLOBALS['connect'], "select keyword from ".$mysqli_table_prefix."keywords where soundex(keyword) = soundex('$word')");
				$max_distance = 100;
				$near_word ="";
				while ($row=mysqli_fetch_row($result)) {
		...
		$query1 = "SELECT distinct link_id, url, title, description,  $fulltxt, size FROM ".$mysqli_table_prefix."links WHERE link_id in ($inlist)";
		...
		while ($row = mysqli_fetch_row($result)) {
			...
			$dom_result = mysqli_query($GLOBALS['connect'], "select domain from ".$mysqli_table_prefix."domains where domain_id='".$domains[$row[0]]."'");
